...
public class ExpandIconView extends View {
	...
	public void setAttrs(AttributeSet attrs){
        ...
        try {
            ...
            mWidth = array.getDimensionPixelSize(R.styleable.ExpandIconView_eiv_width, 0);
            mHeight = array.getDimensionPixelSize(R.styleable.ExpandIconView_eiv_height, 0);
        ...
        paint.setStrokeWidth(Tools.dip2px(2));
        if (roundedCorners) {
	...
	protected void onSizeChanged(int width, int height, int oldWidth, int oldHeight) {
		super.onSizeChanged(mWidth, mHeight, oldWidth, oldHeight);
		calculateArrowMetrics(mWidth, mHeight);
		updateArrowPath();
	}
    ...
    private void calculateArrowMetrics(int width, int height) {
		...
		//}
		//final int arrowWidth = width - 2 * padding;
		//float thickness = (int) (width * THICKNESS_PROPORTION);
		//paint.setStrokeWidth(thickness);

		center.set(width / 2, height / 2);
		left.set(center.x - width / 2, center.y);
		right.set(center.x + width / 2, center.y);
	}
	...
	private void updateArrowPath() {
		...
		centerTranslation = (center.y - tempLeft.y) / 2f;
